...
def main():
    ...
    for g, label in zip(games, game_labels(games)):
        ...
        for me, opp, ha, my_p, opp_p, opp_id in [
            ...
            rows.append({
                ...
                "oppBullpenEff": (beff.get(opp) or {}).get("effERA"),
    ...
    # three-layer bullpen table for the Bullpens tab
    def lay(d):
        return [d.get("era"), d.get("whip"), d.get("kbb"), d.get("hr9"), d.get("ip")] if d else [None]*5
    bullpen_eff = []
    for a, tid, _ in TEAMS:
        v = beff.get(a)
        if not v:
            continue
        bullpen_eff.append([a, lay(v["season"]), lay(v["l14"]), lay(v["l7"]),
...
def write_js(path, date, rows, runsRank, opsRank, bullpen, bullpenEff=None):
...
{row_lines}
  ...
  bullpenEff: {json.dumps(bullpenEff or [], ensure_ascii=False)}
}};
'''
